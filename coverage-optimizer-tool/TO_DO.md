...
| C-3 | Backdate tab | Backdate Projection container (savings dates + the 6-column table) | Your formulas |
...
### C-3 · Backdate tab · Backdate Projection container
- **What:** Monthly Savings Date, Annual Savings Date and the 6-column table (Date, Premium Current, Cumul. Prem. Current, Premium Backdated, Cumul. Prem. Backdated, Difference).
- **Now:** all amber ("yellow, formula later"). The table has no rows because no row rule (period / date range) was given. Final Backdate Date, in the container above, is done.
- **Waiting on:** your formulas and the projection's row rule.
- **Also lights up:** the Summary strip's "Backdate Savings Date" mirrors the Monthly or Annual Savings Date (whichever Payment Frequency picks), so it stays amber until this lands — wire it in `optimizer.js` → `summaryFields` at the same time.
